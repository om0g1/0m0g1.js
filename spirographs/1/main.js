...
function createPoints() {
    for (let t = 0; t < steps; t++) {
        const point = {
            x: Math.floor(R - a)*Math.cos(t) + b*Math.cos(((R-a)/R)*t) + center.x,
            y: Math.floor(R - a)*Math.sin(t) - b*Math.sin(((R-a)/R)*t) + center.y,
        }
        points.push(point);
    }
}
...
colors = generateGradientColors("#00a4eb", "#eba4eb", steps);
createPoints();
drawSpirograph();